    ...
    def run(self):
        ...
        while not self.exit and not RL_MODE:
            ...
            if pressed[pygame.K_F1]:
                ...
                RL_MODE = False
            if math.floor(time_spent) in c:
                # 우현 선회       
                if self.center[1]<195:
                    self.angle_r += -1*0.0005
                    self.angle_r = max(-45, min(self.angle_r, 45))
                    if self.angle_b < -16:
                        self.angle_r = 0
                # 좌현 선회
                elif self.center[1]>205:
                    self.angle_r += 1 * 0.0005
                    self.angle_r = max(-45, min(self.angle_r, 45))
                    if self.angle_b > 16:
                        self.angle_r = 0
            ...
            if not self.ship_alive or self.goal:
                self.goal = False
                self.ship_alive= True
                self.gene += 1
